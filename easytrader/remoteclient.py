...
class RemoteClient:
    ...
    def hangqing(self, name=''):
        ...
        response = self._s.post(self._api + "/hangqing", json=params, timeout=TIMEOUT)
        if response.status_code >= 300:
            raise Exception(response.json()["error"])
        return response.json()
...
class MyRpcClient(RpcClient):
    ...
    # 原版rpc组件中，如果上一个请求后30秒内没有新地请求，会输出一段提示
...
class ZMQSession(object):
    ...
    def post(self, url, json=None, timeout=TIMEOUT):
        name = url.split("/")[-1]

        data, status_code = getattr(self._rpc_client, name)(json)  #调用远程函数

        resp = ZMQResponse(status_code, data)
        return resp

    def get(self, url, json=None, timeout=TIMEOUT):
